...
namespace Service.ServiceImp
{
    public class PostUserManage : RepositoryBase<Domain.SYS_POST_USER>, IService.IPostUserManage
    {
        ...
        public List<Domain.SYS_USER> GetUserListByPostId( string postId )
        {
            try
            {
                string sql = @"select * from sys_user t where exists(select u.fk_userid from sys_post_user u inner join sys_post_department p on u.fk_post_departmentid=p.id where t.id=u.fk_userid and p.fk_post_id in(" + postId + ") group by u.fk_userid)";
        ...
        public bool SavePostUser( int userId, string postId )
        {
            try
            {
                if ( this.IsExist(p=>p.FK_USERID==userId) )
                {
                    ...
                    if ( oldCount.Count==newpostId.Count && oldCount.All(newpostId.Contains) )
                    {
                        return true;
                    }
                    //删除原有关系
                ...
                {
                    //添加现有关系
                    var list = postId.Split( ',' ).Select( item => new Domain.SYS_POST_USER() { FK_USERID = userId, FK_POST_DEPARTMENTID = int.Parse( item ) } ).ToList();
        ...
        public dynamic GetPostNameBySysPostUser( ICollection<Domain.SYS_POST_USER> collection )
        {
            ...
            try
            {
                string sql = @"select d.name+'-'+p.postname as postname,s.id from sys_department d inner join sys_post_department s on d.id=s.fk_department_id inner join sys_post p on p.id=s.fk_post_id where s.id in (" + post_departmentid + ")";
