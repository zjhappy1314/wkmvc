...
namespace Service.ServiceImp
{
    ...
    public class DepartmentManage : RepositoryBase<Domain.SYS_DEPARTMENT>, IService.IDepartmentManage
    {
        ...
        /// 自动创建部门编号
        /// </summary>
        /// <param name="parentCode">上级部门ID注：ID不是Code，数据表已改</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <returns></returns>
        public System.Collections.IList GetDepartmentByDetail()
        {
            var list = RecursiveDepartment( this.LoadAll( null ).ToList() ).Select( p => new { id = p.ID, code = p.CODE, name = GetDepartmentName( p.NAME, p.BUSINESSLEVEL ) } ).ToList();
        ...
        /// <summary>
        /// 根据传入的数字 返回补码后的3位部门编号
        /// </summary>
        /// <param name="dptCode"></param>
        /// <returns></returns>
        private string FormatCode( int dptCode )
        {
            try
            {
                string _strCode = string.Empty;
