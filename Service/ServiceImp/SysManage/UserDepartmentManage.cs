...
namespace Service.ServiceImp
{
    ...
    {
        #region IUserDepartmentManage 成员
        /// <summary>
        ...
        public bool SaveUserDpt( int userId, string dptId )
        {
            try
            {
                ...
                if ( this.IsExist(p=>p.USER_ID==userId) )
                {
                    ...
                    if ( oldCount.Count==newdptid.Count && oldCount.All(newdptid.Contains) )
                    {
                        return true;
                    }
                    //删除原有关系
                ...
                {
                    //添加现有关系
                    var list = dptId.Split( ',' ).Select( item => new Domain.SYS_USER_DEPARTMENT() { DEPARTMENT_ID = item, USER_ID = userId } ).ToList();
